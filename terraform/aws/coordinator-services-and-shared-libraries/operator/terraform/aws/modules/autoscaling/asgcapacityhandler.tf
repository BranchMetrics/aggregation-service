...
resource "aws_s3_object_copy" "asg_capacity_handler_lambda_s3_jar" {
  bucket      = aws_s3_bucket.lambda_package_storage.id
  key         = "app/asg_capacity_handler_lambda.jar"
  source      = "${var.artifacts_s3}${var.asg_capacity_handler_lambda_local_jar}"
}

resource "aws_s3_object_copy" "asg_capacity_handler_lambda_s3_jar_hash" {
  bucket       = aws_s3_bucket.lambda_package_storage.id
  key          = "app/asg_capacity_handler_lambda.jar.hash"
  source       = "${var.artifacts_s3}${var.asg_capacity_handler_lambda_local_jar}.hash"
  content_type = "text/plain"
}

data "aws_s3_bucket_object" "asg_capacity_handler_lambda_s3_jar_hash" {
  depends_on  = [aws_s3_object_copy.asg_capacity_handler_lambda_s3_jar_hash]
  bucket      = aws_s3_bucket.lambda_package_storage.id
  key         = "app/asg_capacity_handler_lambda.jar.hash"
}
...
resource "aws_lambda_function" "asg_capacity_lambda" {
  ...
  source_code_hash = data.aws_s3_bucket_object.asg_capacity_handler_lambda_s3_jar_hash.body
