 ...
 */
resource "aws_s3_object_copy" "frontend_lambdas_package" {
  bucket      = aws_s3_bucket.lambda_package_storage.id
  key         = "app/frontend_lambdas.jar"
  source      = "${var.artifacts_s3}${var.frontend_lambda_local_jar}"
}

resource "aws_s3_object_copy" "frontend_lambdas_package_hash" {
  bucket       = aws_s3_bucket.lambda_package_storage.id
  key          = "app/frontend_lambdas.jar.hash"
  source       = "${var.artifacts_s3}${var.frontend_lambda_local_jar}.hash"
  content_type = "text/plain"
}

data "aws_s3_bucket_object" "frontend_lambdas_package_hash" {
  depends_on  = [aws_s3_object_copy.frontend_lambdas_package_hash]
  bucket      = aws_s3_bucket.lambda_package_storage.id
  key         = "app/frontend_lambdas.jar.hash"
}
...
resource "aws_lambda_function" "get_job_lambda" {
  ...
  source_code_hash = data.aws_s3_bucket_object.frontend_lambdas_package_hash.body
